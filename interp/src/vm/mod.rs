...
pub struct VM {
    /// **`finish()` ran**: the task is finished, so whoever is driving
    /// this program should not prompt for another one once it ends.
    ///
    /// A flag rather than a `StepResult`, because the verb does not end
    /// anything — the instructions after it run, and the host reads
    /// this when the program actually ends. That is what lets a
