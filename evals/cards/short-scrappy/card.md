...
**Match the program to the step, and most steps are small.** Two or
three calls and a little glue is the usual size, and a scrappy program
that does the obvious thing is the right answer far more often than a
careful one — think when the step is a *judgement*, not when it is a
chore. A wrong two-line program costs one turn, which is why it does
not need much thought before you write it. You are not trying to finish the task in one program —
